...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        grad_array = train(args, model, device, train_loader, optimizer, epoch,
        ...
        full_rank_accum = [torch.zeros_like(copy_l) for copy_l in model.parameters() if copy_l.ndim == 4]
        for grad_train in grad_array:
        ...
        print(f"[Epoch {epoch}] gradient norm for each layer is {grad_calc_dict[epoch]}")
        print(f"[Epoch {epoch}] sum of gradient norm is {sum(grad_calc_dict[epoch])}")
        ...
        if epoch % check_freq == 0:
            ...
            # # method 1: calculate for each layer, a model is out of cr if all layers are out of cr
            # ratios = []
            # for new_norm, prev_norm in zip(current_grad_norms, old_grad_norms):
            #     if epoch != 0:
            #         ratio = (abs(prev_norm - new_norm))/(prev_norm)
            #         ratios.append(ratio)
            # # if all layers are out of critical regime, then the whole model is out
            # out_of_critical_regime = len(ratios) != 0 and max(ratios) < threshold
            # print(f"len(ratios) is {len(ratios)}, number of cr layers is {sum(1 for ratio in ratios if ratio > threshold)}")
            # print(f"Epoch {epoch}, is {'' if out_of_critical_regime else 'not'} out of critical regime")

            # method 2: sum the gradient norms across all layers and do the computation based on that
            if epoch != 0:
                ...
                print(f"Epoch {epoch}, ratio is {ratio}")
        else:
