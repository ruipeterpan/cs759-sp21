...
class Recoder(object):
    ...
    def __init_loss_module(self):
        if issubclass(self.loss.__class__, torch.nn.Module):
            ...
            self.loss_module = BCEWithLogitsLoss(
                reduction='sum', **self.loss_params)
        elif self.loss == 'mse':
    ...
    def __init_optimizer(self, lr, weight_decay):
        ...
        for param_name, param in self.model.named_parameters():
            ...
            if param_name in sparse_params_names:
                ...
                sparse_params.append(
                    {'params': param, 'weight_decay': _weight_decay})
            else:
        ...
        if self.optimizer_type == "adam":
            ...
            if len(sparse_params) > 0:
                raise ValueError(
                    'Sparse gradients optimization not supported with adagrad')

            self.optimizer = optim.Adagrad(params, lr=lr)
        elif self.optimizer_type == "sgd":
            if len(sparse_params) > 0:
                raise ValueError(
                    'Sparse gradients optimization not supported with sgd')

            self.optimizer = optim.SGD(params, lr=lr, momentum=0.9)
        elif self.optimizer_type == "rmsprop":
            if len(sparse_params) > 0:
                raise ValueError(
                    'Sparse gradients optimization not supported with rmsprop')
    ...
    def init_from_model_file(self, model_file, local_rank, train_dataloader=None):
        ...
        if not os.path.isfile(model_file):
            ...
            model_saved_state = train_dataloader.load_checkpoint(
                model_file, local_rank)
        else:
        ...
        self.loss_params = model_saved_state.get(
        ...
        self.__sparse_optimizer_state_dict = model_saved_state.get(
    ...
    def __init_training(self, train_dataset, lr,
        ...
        # init model & optimizer with apex once they are both initialized
        self.model, self.optimizer = amp.initialize(
            self.model, self.optimizer, opt_level=self._apex_amp)
    ...
    def train(self, local_rank, train_dataset, val_dataset=None,
        ...
        self.__init_training(train_dataset=train_dataset,
        ...
        if os.path.exists(model_checkpoint_prefix):
            try:
                ...
                if self._enable_gavel_iterator:
                    ...
                    self.init_from_model_file(
                        model_checkpoint_prefix, local_rank)
            except Exception as e:
                print('Could not load from checkpoint: %s' % (e))
        else:
            print('Checkpoint does not exist at %s' %
                  (model_checkpoint_prefix))

        if lr_milestones is not None:
            _last_epoch = - \
                1 if self.current_epoch == 1 else (self.current_epoch - 2)
    ...
    def _train(self, train_dataloader, val_dataloader,
        ...
        for epoch in range(current_epoch, num_epochs + 1):
            ...
            iters_to_process = min(
                iters_per_epoch, num_batches - iters_processed)
    ...
    def __compute_loss(self, input, target):
        ...
        normalization = torch.FloatTensor(
            [target_dense.size(0)]).to(device=self.device)
    ...
    def predict(self, users_interactions, return_input=False):
        ...
        batch_collator = BatchCollator(batch_size=len(
            users_interactions.users), negative_sampling=False)
        ...
        input_dense = torch.sparse.FloatTensor(batch.indices, batch.values, batch.size) \
            .to(device=self.device).to_dense()
        output = self.model(
            input_dense, input_users=batch.users.to(device=self.device))
    ...
    def _evaluate(self, eval_dataset, num_recommendations, metrics, batch_size=1, num_users=None):
        ...
        results = evaluator.evaluate(
            eval_dataset, batch_size=batch_size, num_users=num_users)
        return results
    ...
    def recommend(self, users_interactions, num_recommendations):
        ...
        top_output, top_ind = torch.topk(
            output, num_recommendations, dim=1, sorted=True)
