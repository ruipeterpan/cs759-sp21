...
parser.add_argument('--checkpoint_dir', type=str,
                    default='/home/ruipan/checkpoints',
                    help='Checkpoint dir')
...
class CorpusDataset(torch.utils.data.Dataset):
    def __init__(self, data, batch_size, bptt):
        self._data = data.narrow(
            0, 0, (data.size(0) // batch_size) * batch_size)
        # Evenly divide the data across the bsz batches.
        self._data = self._data.view(
            batch_size, -1).t().contiguous().to(device)
    ...
    def get_input(self, row_idx, col_idx):
        ...
        target = self._data[row_idx+1: row_idx +
                            1+seq_len, col_idx].view(data.size())
        data = torch.cat([data, data.new_zeros(self._bptt - data.size(0))])
        target = torch.cat(
            [target, target.new_zeros(self._bptt - target.size(0))])
        return data, target
...
def load_checkpoint(args, checkpoint_path):
    try:
        ...
        with open(checkpoint_path, 'rb') as f:
            state = torch.load(
                f, map_location='cuda:{}'.format(args.local_rank))
            return state
...
def train(args, cumulative_steps=None, cumulative_time=None):
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        cumulative_steps, cumulative_time, done = train(args, cumulative_steps,
                                                        cumulative_time)
        print('-' * 89)
...
print(f"JCT: {round(time.time() - overall_start, 2)}s")
print("===============")
